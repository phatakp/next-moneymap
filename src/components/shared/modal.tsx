...
export const ModalProvider = ({
  ...
  const closeModal = (id: string) => {
    if (modalId === id) setOpen(false);
  };

  return (
    <ModalContext.Provider value={{ open, setOpen, modalId, closeModal }}>
      {children}
...
export function Modal({ id, children }: { id: string; children: ReactNode }) {
  return <ModalProvider id={id}>{children}</ModalProvider>;
}
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ref={modalRef}
            className={cn(
              "border-border bg-background relative z-50 flex max-h-[90%] min-h-[50%] flex-1 flex-col overflow-y-scroll border md:max-w-[40%] md:rounded-2xl",
              className,
            )}
            initial={{
              opacity: 0,
